...
読む。パッケージとして入っていない場合は `'0.0.0'`、パッケージの外から
読み込まれた場合は `'_._._'` になる。CLI の `-V` / `--version` が出すのも
この値。
...
全サブコマンドに、次の 3 つが共通で付く。

| オプション | 意味 |
|---|---|
| `-d` / `--debug` | ログを DEBUG にする |
| `-h` / `--help` | 使い方を表示する |
| `-V` / `--version` | バージョンを表示する |

`-v` は共通オプションに**含めていない**。`parse` の `--visual` と
`wav` の `--vol` が既に使っているためで、バージョンを見るときは
`-V`（大文字）を使う。
