...
- 要求書の一覧表と同じ #1〜#13 の並びで、各項目について書く
  - 対応の可否（対応した／既に対応済みだった／採らなかった）
  - 実際に何を変えたか、公開 API はどうなったか（呼び出し例）
  - 要求書に「受け入れ条件」があるものは、それを満たしたことの根拠
- **要求と違う判断をした箇所は理由を明記する**。少なくとも次の 3 つ
  - #4: コールバック `on_note` は採らず、DEBUG ログにした
  - #7: `TypedDict` の名前を要求書に合わせて `ParsedMidi` にした
  - #8: `transpose()` の範囲外は `ValueError`（1音でも範囲外なら失敗）
- 利用側の移行に必要な情報を書く
  - タグ `0.0.3` からの変更点のうち、**挙動が変わるもの**
    （`Player` の出力、mixer の初期化タイミング、ロガーの扱い）
  - 新しいタグ（0.1.0 相当）を使う必要があること
- 追加された API の一覧（`write()` / `transpose()` / `stop()` /
  `is_playing()` / `play(block=)` / `close()` / コンテキストマネージャ /
  `format_visual()` / `my_logger.init_handler()`）

### 完了条件

- 要求書の #1〜#13 に漏れなく回答している
